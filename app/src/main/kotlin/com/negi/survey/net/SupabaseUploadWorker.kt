...
class SupabaseUploadWorker(
    ...
    private suspend fun doFileUpload(
        ...
        val prefix = listOf(cfg.pathPrefix.trim('/'), remoteDir.trim('/'))
            .filter { it.isNotBlank() }
            .joinToString("/")

        val objectPath = SupabaseUploader.buildDatedObjectPath(prefix, fileName)

        Log.d(TAG, "doFileUpload: bucket=${cfg.bucket} objectPath=$objectPath file=$filePath size=$size upsert=${inputData.getBoolean(KEY_UPSERT, false)}")
        ...
        return try {
            val res = SupabaseUploader.uploadFile(
                ...
                file = f,
                contentType = inputData.getString(KEY_CONTENT_TYPE) ?: "application/octet-stream",
